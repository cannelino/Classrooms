...
namespace Fusion.Addons.TextureDrawing
{
    ...
    public class TexturePen : NetworkBehaviour
    {
        ...
        {
            base.Spawned();
            // Stored to avoid calling network methods during OnBeforeRender before the object is spawned
            spawned = true;
        }
        ...
        void OnBeforeRender()
        {
            if (spawned && blockableTip != null && blockableTip.RenderPosititionFixTiming == ExtrapolationTiming.DuringUnityOnBeforeRender)
            {
                CheckDrawingPointInsertion();
            }
        }
        ...
        {
            base.Render();
            if (blockableTip == null || blockableTip.RenderPosititionFixTiming == ExtrapolationTiming.DuringFusionRender)
            {
                CheckDrawingPointInsertion();
            }
        }

        void CheckDrawingPointInsertion()
        {
            ...
            if (blockableTip.IsContactAllowed && blockableTip.IsInContact && blockableTip.lastSurfaceInContact != null)
            {
                ...
                if (currentDrawing)
                {
                    ...
                    if (feedback != null )
                    {
                        feedback.PlayAudioAndHapticFeedback(audioType: audioType, audioOverwrite: false, hapticAmplitude: Mathf.Clamp01(hapticAmplitudeFactor * blockableTipPressure), feedbackMode: feedbackMode);
                    }
                }
            } 
