...
namespace Fusion.Addons.BlockingContact
{
    ...
    {
        public const int EXECUTION_ORDER = INetworkController.EXECUTION_ORDER + 10;
        ...
        [Tooltip("Only BlockingSurface's collider matching this layer mask can block this object")]
        ...
        public virtual bool IsContactAllowed => (grabbable == null) || IsGrabbedBySpatialGrabber || ShouldFixGrabberPosition;
        protected virtual bool ShouldFixBlockablePosition => IsInContact && IsContactAllowed;
        protected virtual bool ShouldFixGrabberPosition => grabbable != null && grabbable.CurrentGrabber != null && grabbable.CurrentGrabber.RigPart != null && IsGrabbed;

        public ExtrapolationTiming RenderPosititionFixTiming { 
            get
            {
                if (grabbable != null && grabbable.CurrentGrabber != null)
                {
                    var timing = grabbable.CurrentGrabber.RigPart.RequiredExtrapolationTiming();
                    // Even if the rig does not deal with extrapolation, we have here to choose one extrapolation mode, as the core logic is to change the BlockableTip position after it has been naturally changed
                    if (timing != ExtrapolationTiming.NoExtrapolation)
                    {
                        return timing;
                    }
                }
                return ExtrapolationTiming.DuringUnityOnBeforeRender;
            }
        }
        ...
        protected virtual bool IsGrabbedBySpatialGrabber {
            get {
                ...
                return grabbable != null && grabbable.CurrentGrabber != null && grabbable.CurrentGrabber.RigPart is VisionOsHelpers.NetworkSpatialPointer;
#else
                return false;
#endif
            }
        ...
        [SerializeField] float hapticAmplitudeFactor = 2f;
        ...
        {
            base.Render();
            if (RenderPosititionFixTiming == ExtrapolationTiming.DuringFusionRender)
            {
                FixContactPosition();
            }
            ...
            {
                feedback.PlayAudioAndHapticFeedback(audioType, Mathf.Clamp01(hapticAmplitudeFactor * lastContactDepth), feedbackMode: feedbackMode);
            }

            // stop audio feedback if object is not grabbed (in case there was not triggerExit)
            if (feedbackMode != FeedbackMode.None && feedback != null && IsGrabbed == false)
            {
                feedback.StopAudioFeedback();
            }
        }

        // BeforeRenderOrder is an equivalent of DefaultExecutionOrder for Application.onBeforeRender methods
        [BeforeRenderOrder(BlockableTip.EXECUTION_ORDER)]
        private void OnBeforeRender()
        {
            if (Object == null || Object.IsValid == false) return;
            if (RenderPosititionFixTiming == ExtrapolationTiming.DuringUnityOnBeforeRender)
            {
                FixContactPosition();
            }
        }
        ...
        protected virtual void FixContactPosition()
        {
            ...
            if (ShouldFixBlockablePosition)
            {
                ...
                if (localTipPosition.z > lastSurfaceInContact.maxDepth)
                {
                    ...
                    var fixedPosition = newTipPosition - (tipPositionForBlockableTransform - transform.position);
            ...
            {
                Transform handTransform = grabbable.CurrentGrabber.RigPart.transform;
