...
namespace Fusion.XR.Shared
{
    ...
    public class UserSpawner : MonoBehaviour, IUserSpawner, INetworkRunnerCallbacks
    {
        ...
        private bool _npcSpawnRequested = false;
        ...
        public void OnPlayerJoinedSharedMode(NetworkRunner runner, PlayerRef player)
        {
            ...
            if (player == runner.LocalPlayer && userPrefab != null)
            {
                ...
                NetworkObject networkPlayerObject = runner.Spawn(userPrefab, position: transform.position, rotation: transform.rotation, player, (r, obj) => {
                });
            }
            TrySpawnNpcOnce(runner);
        }
        ...
        public void OnPlayerJoinedHostMode(NetworkRunner runner, PlayerRef player)
        {
            ...
            if (runner.IsServer && userPrefab != null)
            {
                ...
                NetworkObject networkPlayerObject = runner.Spawn(userPrefab, position: transform.position, rotation: transform.rotation, inputAuthority: player, (r, obj) => {
                });
        ...
        #endregion
        // ---------- NPC spawn helper ----------
        private void TrySpawnNpcOnce(NetworkRunner runner)
        {
            if (_npcSpawnRequested) return;
            _npcSpawnRequested = true;

            if (npcSpawner == null)
            {
                Debug.LogWarning("[UserSpawner] NPCSpawner reference is not set. Skipping NPC spawn.");
                return;
            }

            npcSpawner.TrySpawnNPC(runner);
        }
