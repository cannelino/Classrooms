...
namespace Fusion.Addons.VisionOsHelpers
{
    ...
    public class VolumeCameraLocomotion : MonoBehaviour
    {
        ...
        void FindHardwareRig() {
            if(hardwareRig != null && hardwareRig.isActiveAndEnabled == false)
            {
                hardwareRig = null;
            }
            if(hardwareRig == null)
            {
                ...
                planeManagers = new List<ARPlaneManager>(FindObjectsByType<ARPlaneManager>(FindObjectsSortMode.None));
            }
        }
