...
namespace Fusion.Addons.HandsSync.Meta
{
    ...
    internal static class MetaCoreIntegrationCheck
    {
        ...
        {
            if (EditorUserBuildSettings.activeBuildTarget == BuildTarget.VisionOS || EditorUserBuildSettings.activeBuildTarget == BuildTarget.WebGL)
            {
                // Meta Core SDK is incompatible with visionOs or webGL builds, for now: not adding the define check to allow scripts to remove Meta packages if needed
                return;
            }
