...
namespace Fusion.Addons.ExtendedRigSelectionAddon
{
    ...
    public class ExtendedRigSelection : MonoBehaviour, IRigSelection
    {
        ...
        public struct RigKindDescription
        {
            ...
            public IHardwareRig HardwareRig {
                get {
                    if (specificGameObjects == null) return null;
                    foreach (var specificGameObject in specificGameObjects) {
                        if (specificGameObject && specificGameObject.TryGetComponent<IHardwareRig>(out var rig))
                        {
                            return rig;
                        }
                    }
                    if(spawnedObjects != null)
                    {
                        ...
                        {
                            if (spawnedObject && spawnedObject.TryGetComponent<IHardwareRig>(out var rig))
                            {
                                return rig;
                            }
                        }
        ...
        private void Awake()
        {
            ...
            int rigIndex = 0;
            bool scriptingSymbolForcedRig = false;
            foreach (var rigDescription in rigKindDescriptions)
            {
#if FORCE_SELECTING_RIGINDEX_0
                if (rigIndex == 0) scriptingSymbolForcedRig = true;
#endif
#if FORCE_SELECTING_RIGINDEX_1
                if (rigIndex == 1) scriptingSymbolForcedRig = true;
#endif
#if FORCE_SELECTING_RIGINDEX_2
                if (rigIndex == 2) scriptingSymbolForcedRig = true;
#endif
#if FORCE_SELECTING_RIGINDEX_3
                if (rigIndex == 3) scriptingSymbolForcedRig = true;
#endif
        ...
        void SelectRig(RigKindDescription rigDescription)
        {
            ...
            else if(rigDescription.useSpecificNetworkRig && selectedConnexionHandler && selectedConnexionHandler.TryGetComponent(out IUserSpawner userSpawner))
            {
                userSpawner.UserPrefab = rigDescription.specificNetworkRig;
            }
