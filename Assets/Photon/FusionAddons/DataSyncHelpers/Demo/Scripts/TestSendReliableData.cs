...
public class TestSendReliableData : NetworkBehaviour, INetworkRunnerCallbacks
{
    ...
    void INetworkRunnerCallbacks.OnReliableDataReceived(NetworkRunner runner, PlayerRef player, ReliableKey key, ReadOnlySpan<byte> receivedData) { 
        OnDataReceived(player, key, receivedData.ToArray());      
    }
#else
    ...
    {
        OnDataReceived(player, key, receivedSegment.Array);
    }
#endif

    #endregion

    public void OnDataReceived(PlayerRef player, ReliableKey key, byte[] data)
    {
        key.GetInts(out var key0, out var key1, out var key2, out var key3);
        if (key0 == (int)Object.Id.Raw)
        {
