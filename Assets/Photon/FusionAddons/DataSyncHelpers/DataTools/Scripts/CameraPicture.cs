...
namespace Fusion.Addons.DataSyncHelpers {
    /**
     * Send a picture to remote users through Fusion data streaming API, a apply the uncompressed data to a renderer
     */
    public class CameraPicture : StreamSynchedBehaviour
    {
        Texture2D texture;

        [Tooltip("Renderer to which the captured texture will be applied. Not used if a TargetRenderTexture is found")]
        ...
        [SerializeField] private bool playSoundWhenProgressStart = true;
#endif
...
#if XRSHARED_ADDON_AVAILABLE
            if (feedback == null) feedback = GetComponentInParent<IFeedbackHandler>();
#endif
        }
        ...
        public override void Spawned()
        {
            base.Spawned();
#if XRSHARED_ADDON_AVAILABLE
            if (playSoundWhenProgressStart && feedback != null && feedback.IsAudioFeedbackIsPlaying() == false)
            {
        ...
        public void SetPictureTexture(RenderTexture renderTexture)
        {
            if (Object.HasStateAuthority && texture == null)
            {
                ...
                var pictureData = ByteArrayTools.TextureData(renderTexture);
                Log($"SetPictureTexture => Send {pictureData.Length} bytes", level: LogLevel.Details);
                Send(pictureData);
            }
        }
        ...
        protected override void OnDataProgress(float progress)
        {
            ...
            if (loadingGameObject && loadingGameObject.activeInHierarchy == false)
            {
        ...
        public override void OnDataChunkReceived(byte[] data, PlayerRef source, float time)
        {
            ...
            if (loadingGameObject && loadingGameObject.activeInHierarchy)
            {
                loadingGameObject.SetActive(false);
            }
#if XRSHARED_ADDON_AVAILABLE
            if (playSoundWhenProgressStart && feedback != null && feedback.IsAudioFeedbackIsPlaying() == true)
